...
const getAirJordans = catchAsync(async (req, res) =>{
    ...
    console.log(req.query)
    if (gender) {
    const shoesByGender = await Shoe.find({brand:"Air Jordan", gender: gender});
    res.send(shoesByGender)
    } else {
    res.send(airJordans);
    }
...
const getYeezyShoes = catchAsync(async (req, res) =>{
    ...
    console.log(req.query)
    if (gender) {
    const shoesByGender = await Shoe.find({name: {$regex:"Yeezy"}, gender: gender});
    res.send(shoesByGender)
    } else {
    res.send(yeezyShoes);
    }
...
module.exports = { getShoes, getNikeShoes, getAdidasShoes, getAirJordans, getYeezyShoes };
